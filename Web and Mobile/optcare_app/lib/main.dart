...
String resolveFrameImageUrl(String? rawUrl) {
  ...
  if (cleaned.startsWith('/')) {
    return 'http://10.0.2.2:5000$cleaned';
  }
  ...
  if (cleaned.contains('/')) {
    return 'http://10.0.2.2:5000/${cleaned.replaceFirst(RegExp(r'^/+'), '')}';
  }

  return 'http://10.0.2.2:5000/uploads/${cleaned.replaceFirst(RegExp(r'^/+'), '')}';
}
...
class _AuthScreenState extends State<AuthScreen> {
  ...
  Future<void> _submit() async {
    ...
    try {
      // Use PHP backend endpoints (login.php / register.php) in optcare_api
      final endpoint = _isLogin ? '/login.php' : '/register.php';
      final uri = Uri.parse('$apiBaseUrl$endpoint');

      final selectedGenderValue = _selectedGender;
      final dobValue = _selectedDateOfBirth?.toIso8601String().split('T').first;
      ...
      final response = await http
          .post(
            ...
            body: jsonEncode(
              _isLogin
                  ? {
                      // PHP backend expects `email` + `password` for login
                      'email': payload['email'],
      ...
      try {
        ...
        // If server returned unexpected body, keep data empty to trigger error handling
        data = {};
      }

      if (!mounted) return;

      // Support Node backend response: { token, user } or registration success { success: true }
      if (response.statusCode == 200 &&
        ...
        if (_isLogin) {
          // Handle multiple response shapes: PHP returns { success, data }, Node may return { token, user }
          ...
          if (user != null) {
            profileData['patient_id'] =
                user['patient_id'] ?? user['user_id'] ?? user['id'];
            profileData['email'] = user['email'] ?? '';

            final rawDob =
                user['date_of_birth'] ?? user['dob'] ?? user['birth_date'];
            final dobDate = rawDob is String && rawDob.trim().isNotEmpty
                ? DateTime.tryParse(rawDob)
                : null;
            final derivedAge = dobDate == null ? null : _calculateAge(dobDate);
            profileData['date_of_birth'] = rawDob;
            profileData['age'] = derivedAge;

            final rawGender = user['gender'] ?? user['sex'] ?? '';
            profileData['gender'] = rawGender.toString().trim().isEmpty
                ? 'Male'
                : rawGender.toString();
            profileData['created_at'] =
                ...
                user['last_visit'] ??
                '';
            // Prefer explicit first/last names, fallback to splitting a full name
            profileData['first_name'] = user['first_name'] ?? '';
            profileData['last_name'] = user['last_name'] ?? '';
            if ((profileData['first_name'] as String).isEmpty) {
              final full = (user['full_name'] ?? user['name'] ?? '') as String;
              if (full.isNotEmpty) {
                final parts = full.split(' ');
                profileData['first_name'] = parts.isNotEmpty
                    ? parts.first
                    : full;
                profileData['last_name'] = parts.length > 1
          ...
          Navigator.of(context).pushReplacement(
            MaterialPageRoute(
              builder: (_) => DashboardScreen(profileData: profileData),
            ),
          );
        }
        // For registration (data['success'] == true) we show the message and stay on the auth screen
        if (!_isLogin && data['success'] == true) {
          // show message already shown; optionally clear form
        ...
        // Prefer `message` then `error` fields from server
        final msg =
            (data['message'] ?? data['error'])?.toString() ?? 'Request failed';
        ScaffoldMessenger.of(
          context,
...
class _DashboardScreenState extends State<DashboardScreen> {
  ...
  Future<void> _refreshProfile() async {
    try {
      ...
      final uri = Uri.parse('$apiBaseUrl/get_profile.php');
      final response = await http
          .post(
            uri,
            headers: jsonHeaders,
            body: jsonEncode({'patient_id': pid}),
          )
          .timeout(const Duration(seconds: 15));
      ...
      if (data['success'] == true) {
        ...
        debugPrint(
          'Profile refresh failed: ${data['message'] ?? data['error']}',
        );
      }
    } catch (e) {
      debugPrint('Unable to refresh profile: $e');
    }
  }
  ...
  Future<void> _loadAppointments() async {
    ...
    try {
      if (_profile['patient_id'] == null ||
          _profile['patient_id'].toString().trim().isEmpty) {
        setState(() => _appointments = []);
        return;
      }

      final uri = Uri.parse('$apiBaseUrl/get_appointments.php');
      final response = await http
          .post(
            uri,
            headers: jsonHeaders,
            body: jsonEncode({'patient_id': _profile['patient_id']}),
          )
          .timeout(const Duration(seconds: 15));
      ...
      if (data['success'] == true) {
        ...
        setState(() {
          _appointments = appointmentsData is List
              ? appointmentsData
              : const [];
        });
        return;
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Unable to load appointments: $e')),
      );
    } finally {
      if (mounted) {
  ...
  String _resolveFrameImageUrl(String? rawUrl) {
    return resolveFrameImageUrl(rawUrl);
  }
  ...
  Future<void> _loadFrames() async {
    ...
    final uri = Uri.parse('$apiBaseUrl/get_frames.php');
    debugPrint('FRAME API REQUEST: $uri');

    try {
      final response = await http.get(uri).timeout(const Duration(seconds: 20));
      debugPrint('FRAME API STATUS: ${response.statusCode}');
      debugPrint('FRAME API BODY: ${response.body.length} bytes');
      debugPrint(
        'FRAME API BODY (snippet): ${response.body.substring(0, response.body.length > 500 ? 500 : response.body.length)}',
      );

      if (response.statusCode != 200) {
        throw Exception(
          'Failed to load frame inventory (${response.statusCode})',
        );
      }

      final decoded = jsonDecode(response.body);
      final payload = decoded is Map
          ? decoded['data'] ?? decoded['frames'] ?? decoded
          : decoded;

      debugPrint('FRAME API PAYLOAD TYPE: ${payload.runtimeType}');
      if (payload is! List) {
        // Provide actionable error message for debugging on-device
        final snippet = response.body.length > 800
            ? response.body.substring(0, 800)
            : response.body;
        throw Exception(
          'Unexpected frame payload type: ${payload.runtimeType}. Body snippet: $snippet',
        );
      }
      ...
      debugPrint('FRAME API ERROR: $e');
      debugPrint('FRAME API STACK: $st');
      if (!mounted) return;
      setState(() {
        _framesError = true;
        _framesErrorMessage = 'Unable to load frame models right now.';
      });
    } finally {
      if (mounted) {
  ...
  void _showFrameDetails(FrameModel frame) {
    final imageUrl = _resolveFrameImageUrl(frame.imageUrl);
