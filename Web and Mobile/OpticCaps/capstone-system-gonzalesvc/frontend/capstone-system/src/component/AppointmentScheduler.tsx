...
export default function AppointmentScheduler({ preSelectedId, clearPreSelected, onNavigate }: AppointmentSchedulerProps) {
  ...
  const takenSlots = useMemo(() => {
    return appointments
      .filter(a => a.appointment_status !== 'Cancelled' && a.appointment_status !== 'Requested' && a.appointment_status !== 'Pending')
      .map(a => ({
  ...
  const loadSchedule = async () => {
    try {
      ...
      const approved = data.filter((a: Appointment) => !['Requested', 'Pending', 'Cancelled'].includes(a.appointment_status || ''));
      const mobileReqs = data.filter((a: Appointment) => ['Requested', 'Pending'].includes(a.appointment_status || ''));
  ...
  const handleAcceptRequest = async (appointment: Appointment) => {
    try {
      ...
      await appointmentService.updateStatus(targetId, 'Confirmed');
      loadSchedule();
    } catch (err: any) {
                    ...
                    <div key={req.id || req.appointment_id} className="p-3 rounded-lg border border-slate-100 bg-slate-50 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 text-xs">
                      ...
                      <div className="flex items-center space-x-2 self-end sm:self-center">
                        <button 
                          ...
                          className="flex items-center space-x-1 px-2.5 py-1.5 bg-red-100 hover:bg-red-200 text-red-700 font-medium rounded-lg shadow-sm transition-colors"
                        >
                          <X size={12} />
                          <span>Cancel</span>
                        </button>
